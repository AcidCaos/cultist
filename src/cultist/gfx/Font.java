...
public class Font {
    ...
    public static void render(Graphics g, String msg, int x, int y, boolean center) {
        int SHEET_FONT_Y = 30;
        ...
        for (int i = 0; i < msg.length(); i++) {
            ...
            if (ix >= 0) {
                int xx = x + i * CHARACTER_WIDTH;
                if(center) xx = xx - (CHARACTER_WIDTH * msg.length())/2;
                g.drawImage(Assets.sheet.crop(ix % SHEET_FONT_WIDTH, SHEET_FONT_Y + ix / SHEET_FONT_WIDTH, 1, 1),
                        xx, y, CHARACTER_WIDTH, CHARACTER_HEIGHT, null);
            }
        }
    }
